...
>
> setState()메서드는 파라미터로 업데이트할 state가 포함된 객체를 받을 수도 있고, 함수를 받을 수도 있다. 이 때 함수를 전달받는 setState 메서드를 함수형 setState라고 한다.
>
> ```javascript
> this.setState((state, props) => {someState: state.someState + 1})
> ```
>
> > setState의 두번째 파라미터로 콜백 함수가 있을 경우 콜백 함수의 실행이 끝난 후 리렌더링 된다. 없을 경우 상태가 변화하면 리렌더링된다.
> >
> > updater: (state, props) => stateChange
> >
> > updater는 state와 props를 인자로 받는 state를 변화시키는 함수로, 이 때 state와 props는 최신상태로 보장된다.
>
> 
>
...
> ```
>
> 그러나 여러 함수형 setState를 쓰면! 리액트는 객체를 merge하는 대신 setState의 updater 함수를 **호출된 순서대로** Event Queue에 넣는다.
>
